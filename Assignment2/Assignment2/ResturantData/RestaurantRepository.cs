...
namespace Assignment2.ResturantData
{
    public class RestaurantRepository : BaseTableRepository
    {
        ...
        public List<Restaurant> Query(string city, string name)
        {
            var tableClient = GetTableClient();
        ...
        public List<Restaurant> Query(string city, string name, string cuisine)
        {
            var tableClient = GetTableClient();
            var restaurants = tableClient.Query<Restaurant>(x => x.City == city && x.RowKey == name && x.NameOfCuisine == cuisine);
